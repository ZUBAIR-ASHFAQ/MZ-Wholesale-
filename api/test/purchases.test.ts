...
/** Verifies alternate-unit conversion cannot overflow the base inventory quantity field. */
test("rejects a purchase base quantity above the inventory database range", () => {
  assert.throws(
    () =>
      calculatePurchase(
        [
    ...
    (error: unknown) =>
      ...
      error.message === "Purchase item base quantity is too large for inventory.",
  );
});
