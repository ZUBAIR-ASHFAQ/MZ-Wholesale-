...
/** Verifies signed decimal parsing keeps fractional stock shortages negative. */
test("stock-count decimal parsing preserves negative fractional differences", async () => {
  const source = await readSource(inventoryServicePath);
  const helperSource = source.slice(
    source.indexOf("function decimalToScaledInteger"),
    source.indexOf(
      "/** Converts a scaled integer back",
      source.indexOf("function decimalToScaledInteger"),
    ),
  );
  ...
  assert.match(helperSource, /value\.startsWith\("-"\)/);
  assert.match(helperSource, /negative \? value\.slice\(1\) : value/);
  assert.match(helperSource, /return negative \? -scaled : scaled/);
  assert.match(
    confirmationSource,
