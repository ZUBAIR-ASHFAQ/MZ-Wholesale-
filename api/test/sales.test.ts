...
test("sale calculation rejects a base quantity above the inventory database range", () => {
  assert.throws(
    () =>
      calculateSale(
        [
    ...
    (error: unknown) =>
      ...
      error.message === "Sale item base quantity is too large for inventory.",
  );
});
