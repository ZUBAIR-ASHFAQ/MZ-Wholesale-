...
const salesStatusSchema = z.enum(["DRAFT", "HELD", "CONFIRMED", "CANCELLED"]);
const editableSalesStatusSchema = z.enum(["DRAFT", "HELD"]);
const createSalesStatusSchema = z.enum(["DRAFT", "HELD", "CONFIRMED"]);
...
/** Enforces the one matching account required by a sale payment split. */
...
/** Rejects repeated cash/bank accounts inside one initial sale payment. */
...
const saleItemInputSchema = z
  .object({
    ...
    manualUnitPrice: positiveMoneySchema,
...
/** Validates sale-list filters and pagination. */
...
/** Validates a new draft, held, or immediately confirmed sale request. */
export const createSaleSchema = z
  .object({
    ...saleFields,
    status: createSalesStatusSchema.default("DRAFT"),
    initialPayment: initialPaymentSchema.optional(),
  })
  .strict()
  .superRefine((input, context) => {
    if (input.status !== "CONFIRMED" && input.initialPayment) {
      context.addIssue({
        code: "custom",
        path: ["initialPayment"],
        message: "An initial payment can only be recorded when the sale is confirmed.",
      });
    }
  });

/** Validates editable business fields for a DRAFT or HELD sale. */
...
/** Validates the optional cash/bank payment supplied when confirming a saved sale. */
...
/** Validates the optional note supplied when cancelling an unconfirmed sale. */
