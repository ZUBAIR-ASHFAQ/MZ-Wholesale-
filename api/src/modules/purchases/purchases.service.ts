...
function calculateBaseQuantity(
  ...
  const baseQuantity = scaledIntegerToDecimal(baseQuantityUnits, QUANTITY_SCALE);
  if (!isQuantityWithinDatabaseRange(baseQuantity)) {
    throw purchaseCalculationError(
      "Purchase item base quantity is too large for inventory.",
      "items",
    );
  }
