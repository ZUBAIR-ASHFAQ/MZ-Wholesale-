...
export function SupplierPaymentDetailPage({
  ...
  /** Reverses the supplier payment while reusing one idempotency key across retries. */
    ...
    <section>
      ...
      <section className="management-card">
        <dl className="detail-list">
          ...
          <div><dt>Total amount</dt><dd>PKR {trimDecimalZeros(payment.totalAmount)}</dd></div>
          <div><dt>Status</dt><dd>{payment.status === "REVERSED" ? "Reversed" : "Confirmed"}</dd></div>
          <div><dt>Supplier payable</dt><dd>{payment.supplierBalance !== undefined ? `PKR ${trimDecimalZeros(payment.supplierBalance)}` : "—"}</dd></div>
              ...
              <tbody>
                {payment.splits.map((split) => (
                  <tr key={split.id}>
                    ...
                    <td>PKR {trimDecimalZeros(split.amount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="management-card">
              ...
              <tbody>
                {payment.allocations.map((allocation) => (
                  <tr key={allocation.id}>
                    ...
                    <td>PKR {trimDecimalZeros(allocation.amount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {payment.supplierPayableAmount && payment.supplierPayableAmount !== "0.00" ? (
          <p><strong>Existing supplier payable payment:</strong> PKR {trimDecimalZeros(payment.supplierPayableAmount)}</p>
        ) : null}
      </section>
