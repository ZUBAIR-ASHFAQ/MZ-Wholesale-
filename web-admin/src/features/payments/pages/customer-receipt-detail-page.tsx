    ...
    <section>
      ...
      <section className="management-card">
        <dl className="detail-list">
          ...
          <div><dt>Total amount</dt><dd>PKR {trimDecimalZeros(receipt.totalAmount)}</dd></div>
          <div><dt>Status</dt><dd>{receipt.status === "REVERSED" ? "Reversed" : "Confirmed"}</dd></div>
          <div><dt>Customer balance</dt><dd>{receipt.customerBalance !== undefined ? `PKR ${trimDecimalZeros(receipt.customerBalance)}` : "—"}</dd></div>
      ...
      <section className="management-card">
        ...
        {receipt.customerDueAmount && receipt.customerDueAmount !== "0.00" ? (
          <p><strong>Existing customer due payment:</strong> PKR {trimDecimalZeros(receipt.customerDueAmount)}</p>
        ) : null}
      </section>
