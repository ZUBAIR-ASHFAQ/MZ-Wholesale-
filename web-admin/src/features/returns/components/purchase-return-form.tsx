...
import { currentBusinessDate, formatMoney, trimDecimalZeros, formatQuantity } from "../../../lib/utils.ts";
  ...
  async function savePurchaseReturn(values: PurchaseReturnFormValues): Promise<void> {
    ...
    validatedValues.items.forEach((item, index) => {
      ...
      if (exceedsAvailableQuantity(item.quantity, remainingQuantity)) {
        availableQuantityErrors[index] = `Return quantity cannot exceed remaining returnable quantity (${formatQuantity(remainingQuantity)}).`;
      } else if (exceedsAvailableQuantity(item.quantity, currentStockQuantity)) {
        availableQuantityErrors[index] = `Return quantity cannot exceed current sellable stock (${formatQuantity(currentStockQuantity)}).`;
      }
    });
            ...
            >
              ...
              {confirmedPurchases.map((purchase) => (
                <option key={purchase.id} value={purchase.id}>
                  {purchase.purchaseNumber ?? "Confirmed purchase"} · {purchase.purchaseDate} · PKR {trimDecimalZeros(purchase.totalAmount)}
                </option>
              ))}
            </select>
            ...
            <table className="ui-table">
              ...
              <tbody>
                {returnItems.map((returnItem, index) => {
                  ...
                  const availability = returnAvailabilityByItemId.get(purchaseItem.id);
                  const returnedQuantity = availability?.returnedQuantity ?? "0";
                  const remainingQuantity = availability?.remainingReturnableQuantity ?? purchaseItem.quantity;
                  const currentStockQuantity = availability?.currentStockQuantity ?? "0";
                  ...
                  return (
                    <tr key={purchaseItem.id}>
                      ...
                      <td>{formatQuantity(purchaseItem.quantity)}</td>
                      <td>{formatQuantity(returnedQuantity)}</td>
                      <td>{formatQuantity(remainingQuantity)}</td>
                      <td>{formatQuantity(currentStockQuantity)}</td>
                            ...
                            onChange={(event) => {
                              ...
                              setQuantityErrors((currentErrors) => {
                                ...
                                if (exceedsAvailableQuantity(quantity, remainingQuantity)) {
                                  nextErrors[index] = `Return quantity cannot exceed remaining returnable quantity (${formatQuantity(remainingQuantity)}).`;
                                } else if (exceedsAvailableQuantity(quantity, currentStockQuantity)) {
                                  nextErrors[index] = `Return quantity cannot exceed current sellable stock (${formatQuantity(currentStockQuantity)}).`;
                                } else {
