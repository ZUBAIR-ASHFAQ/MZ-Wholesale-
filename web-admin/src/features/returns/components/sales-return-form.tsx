...
import { currentBusinessDate, trimDecimalZeros, formatQuantity } from "../../../lib/utils.ts";
  ...
  async function saveSalesReturn(values: SalesReturnFormValues): Promise<void> {
    ...
    validatedValues.items.forEach((item, index) => {
      ...
      if (saleItem && exceedsSoldQuantity(item.quantity, saleItem.quantity)) {
        soldQuantityErrors[index] = `Return quantity cannot exceed sold quantity (${formatQuantity(saleItem.quantity)}).`;
      }
    });
            ...
            >
              ...
              {confirmedSales.map((sale) => (
                <option key={sale.id} value={sale.id}>
                  {sale.invoiceNumber ?? "Confirmed sale"} · {sale.invoiceDate} · PKR {trimDecimalZeros(sale.totalAmount)}
                </option>
              ))}
            </select>
                ...
                {returnItems.map((returnItem, index) => {
                  ...
                  return (
                    <tr key={saleItem.id}>
                      ...
                      <td>{formatQuantity(saleItem.quantity)}</td>
                      <td>PKR {trimDecimalZeros(saleItem.manualUnitPrice)}</td>
                      <td>
                            ...
                            onChange={(event) => {
                              ...
                              setQuantityErrors((currentErrors) => {
                                ...
                                if (exceedsSoldQuantity(quantity, saleItem.quantity)) {
                                  nextErrors[index] = `Return quantity cannot exceed sold quantity (${formatQuantity(saleItem.quantity)}).`;
                                } else {
