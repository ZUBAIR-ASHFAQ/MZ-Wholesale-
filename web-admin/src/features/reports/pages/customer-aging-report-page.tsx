          ...
          <>
            <div className="summary-grid">
              <article className="summary-card">
                <span>0–30 days</span>
                <strong>PKR {trimDecimalZeros(report.totals.bucket0To30)}</strong>
              </article>
              <article className="summary-card">
                <span>31–60 days</span>
                <strong>PKR {trimDecimalZeros(report.totals.bucket31To60)}</strong>
              </article>
              <article className="summary-card">
                <span>61–90 days</span>
                <strong>PKR {trimDecimalZeros(report.totals.bucket61To90)}</strong>
              </article>
              <article className="summary-card">
                <span>90+ days</span>
                <strong>PKR {trimDecimalZeros(report.totals.bucket90Plus)}</strong>
              </article>
              <article className="summary-card">
                <span>Total outstanding</span>
                <strong>PKR {trimDecimalZeros(report.totals.totalOutstanding)}</strong>
              </article>
            </div>
                ...
                <tbody>
                  {report.items.map((row) => (
                    <tr key={row.customerId}>
                      ...
                      <td>{row.phone ?? "-"}</td>
                      <td>PKR {trimDecimalZeros(row.bucket0To30)}</td>
                      <td>PKR {trimDecimalZeros(row.bucket31To60)}</td>
                      <td>PKR {trimDecimalZeros(row.bucket61To90)}</td>
                      <td>PKR {trimDecimalZeros(row.bucket90Plus)}</td>
                      <td>PKR {trimDecimalZeros(row.totalOutstanding)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
