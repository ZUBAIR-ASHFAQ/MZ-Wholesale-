    ...
    <section>
      ...
      <section className="management-card">
        <dl className="detail-list">
          ...
          <div><dt>Subtotal</dt><dd>PKR {trimDecimalZeros(purchase.subtotalAmount)}</dd></div>
          <div><dt>Item discounts</dt><dd>PKR {trimDecimalZeros(purchase.itemDiscountTotal)}</dd></div>
          <div><dt>Invoice discount</dt><dd>PKR {trimDecimalZeros(purchase.invoiceDiscountAmount)}</dd></div>
          <div><dt>Extra cost</dt><dd>PKR {trimDecimalZeros(purchase.extraCostAmount)}</dd></div>
          <div><dt>Total</dt><dd>PKR {trimDecimalZeros(purchase.totalAmount)}</dd></div>
          <div>
            <dt>Initial paid</dt>
            <dd>{purchase.initialPaidAmount === null ? "—" : `PKR ${trimDecimalZeros(purchase.initialPaidAmount)}`}</dd>
          </div>
          <div>
            <dt>Initial due</dt>
            <dd>{purchase.initialDueAmount === null ? "—" : `PKR ${trimDecimalZeros(purchase.initialDueAmount)}`}</dd>
          </div>
          <div>
            <dt>Current outstanding</dt>
            <dd>{detail.currentOutstandingAmount === null ? "—" : `PKR ${trimDecimalZeros(detail.currentOutstandingAmount)}`}</dd>
          </div>
            ...
            <tbody>
              {detail.items.map((item) => {
                ...
                return (
                  ...
                  <td>{item.unitNameSnapshot} ({formatQuantity(item.conversionToBaseSnapshot)} base)</td>
                  <td>{formatQuantity(item.quantity)}</td>
                  {isConfirmed ? <td>{formatQuantity(availability?.returnedQuantity ?? "0")}</td> : null}
                  {isConfirmed ? <td>{formatQuantity(availability?.remainingReturnableQuantity ?? item.quantity)}</td> : null}
                  {isConfirmed ? <td>{formatQuantity(availability?.currentStockQuantity ?? "0")}</td> : null}
                  <td>{formatQuantity(item.baseQuantity)}</td>
                  <td>PKR {trimDecimalZeros(item.unitCost)}</td>
                  <td>PKR {trimDecimalZeros(item.itemDiscountAmount)}</td>
                  <td>PKR {trimDecimalZeros(item.lineTotal)}</td>
                  <td>PKR {trimDecimalZeros(item.allocatedExtraCost)}</td>
              ...
              <tbody>
                {detail.payments.map((payment) => (
                  <tr key={payment.paymentId}>
                    ...
                    <td>PKR {trimDecimalZeros(payment.totalAmount)}</td>
                    <td>PKR {trimDecimalZeros(payment.allocatedAmount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
