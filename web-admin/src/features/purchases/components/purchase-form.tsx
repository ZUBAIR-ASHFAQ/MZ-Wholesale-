...
import { currentBusinessDate, trimDecimalZeros } from "../../../lib/utils.ts";
...
function initialItemRows(purchaseDetail?: PurchaseDetail): PurchaseItemRow[] {
  ...
  return purchaseDetail.items.map((item) => ({
    ...
    quantity: trimDecimalZeros(item.quantity),
    unitCost: trimDecimalZeros(item.unitCost),
    itemDiscountAmount: trimDecimalZeros(item.itemDiscountAmount),
  }));
}
...
function PurchaseItemEditor({
  ...
  const units =
    product?.units.filter((unit) => unit.isActive || unit.id === row.productUnitId) ?? [];
  const selectedUnit = units.find((unit) => unit.id === row.productUnitId);
    ...
    <div className="purchase-item-row">
        ...
        >
          ...
          {units.map((unit) => (
            <option key={unit.id} value={unit.id}>
              ...
              {unit.isBaseUnit
                ? " (base)"
                : ` — 1 = ${trimDecimalZeros(unit.conversionToBase)} ${product?.baseUnitName ?? "base units"}`}
        ...
        {selectedUnit && product ? (
          <small>
            {selectedUnit.isBaseUnit
              ? `Inventory is maintained in ${product.baseUnitName}.`
              : `1 ${selectedUnit.unitName} = ${trimDecimalZeros(selectedUnit.conversionToBase)} ${product.baseUnitName}; inventory is maintained in ${product.baseUnitName}.`}
          </small>
        ) : null}
      ...
      <label className="ui-field">
        ...
        <input
          ...
          placeholder="0"
          value={row.itemDiscountAmount ?? "0"}
        />
...
export function PurchaseForm({
  ...
  const [invoiceDiscountAmount, setInvoiceDiscountAmount] = useState(
    trimDecimalZeros(purchase?.invoiceDiscountAmount ?? "0"),
  );
  const [extraCostAmount, setExtraCostAmount] = useState(trimDecimalZeros(purchase?.extraCostAmount ?? "0"));
