    ...
    <section className="sale-invoice-page">
      ...
      <section className="management-card">
        <dl className="detail-list">
          ...
          <div><dt>Subtotal</dt><dd>PKR {trimDecimalZeros(sale.subtotalAmount)}</dd></div>
          <div><dt>Item discounts</dt><dd>PKR {trimDecimalZeros(sale.itemDiscountTotal)}</dd></div>
          <div><dt>Invoice discount</dt><dd>PKR {trimDecimalZeros(sale.invoiceDiscountAmount)}</dd></div>
          <div><dt>Total</dt><dd>PKR {trimDecimalZeros(sale.totalAmount)}</dd></div>
          <div><dt>Initial paid</dt><dd>{sale.initialPaidAmount === null ? "—" : `PKR ${trimDecimalZeros(sale.initialPaidAmount)}`}</dd></div>
          <div><dt>Initial due</dt><dd>{sale.initialDueAmount === null ? "—" : `PKR ${trimDecimalZeros(sale.initialDueAmount)}`}</dd></div>
          <div><dt>Current outstanding</dt><dd>{detail.currentOutstandingAmount === null ? "—" : `PKR ${trimDecimalZeros(detail.currentOutstandingAmount)}`}</dd></div>
          {sale.confirmedAt ? (
            ...
            <tbody>
              {detail.items.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>PKR {trimDecimalZeros(item.manualUnitPrice)}</td>
